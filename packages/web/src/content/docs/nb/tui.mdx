...
SumoCode gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre SumoCode starter TUI for gjeldende katalog.
...
Når du bruker SumoCode TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til SumoCode. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt SumoCode. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `sumocode.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge SumoCode venter etter leader key. Standard er `2000`.

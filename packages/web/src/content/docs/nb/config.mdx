...
Du kan konfigurere SumoCode ved å bruke en JSON konfigurasjonsfil.

---

## Format

SumoCode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global konfigurasjon** (`~/.config/sumocode/sumocode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`SUMOCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`sumocode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.opencode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`SUMOCODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.opencode` og `~/.config/sumocode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale SumoCode-konfigurasjon i `~/.config/sumocode/sumocode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `sumocode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når SumoCode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `SUMOCODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `SUMOCODE_CONFIG_DIR`
...
Konfigurasjonsfilen har et skjema som er definert i [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage SumoCode-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i SumoCode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver SumoCode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/sumocode/agents/` eller `.sumocode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil SumoCode falle tilbake til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/sumocode/commands/` eller `.sumocode/commands/`. [Les mer her](/docs/commands).
...
SumoCode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater SumoCode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider SumoCode med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.sumocode/plugins/` eller `~/.config/sumocode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense SumoCode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.

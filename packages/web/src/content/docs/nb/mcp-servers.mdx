...
Du kan legge til eksterne verktøy i SumoCode ved å bruke _Model Context Protocol_ eller MCP. SumoCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [SumoCode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="sumocode.jsonc" {6}
{
...
SumoCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil SumoCode:
...
Hvis serveren krever autentisering, vil SumoCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `sumocode mcp auth <server-name>`.
...
```bash
sumocode mcp auth my-oauth-server
```
...
```bash
sumocode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil SumoCode lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
sumocode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i SumoCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom SumoCode-konfigurasjonen som alle andre verktøy.
...
```json title="sumocode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
sumocode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble SumoCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver SumoCode-samtale oppført som en separat økt.

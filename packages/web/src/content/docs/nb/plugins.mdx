...
description: Skriv dine egne programtillegg for å utvide SumoCode.
---

Plugins lar deg utvide SumoCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til SumoCode.
...
1. Global konfigurasjon (`~/.config/sumocode/sumocode.json`)
2. Prosjektkonfigurasjon (`sumocode.json`)
3. Global plugin-katalog (`~/.config/sumocode/plugins/`)
...
SumoCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En SumoCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide SumoCode.
...
:::note
Hvis du bruker SumoCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre SumoCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til SumoCode:
...
`tool`-hjelperen lager et tilpasset verktøy som SumoCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for SumoCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://sumocode.ai/docs/sdk) for detaljer.

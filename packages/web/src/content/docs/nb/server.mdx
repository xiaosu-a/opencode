...
Kommandoen `opencode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en SumoCode-klient kan bruke.
...
Sett `SUMOCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `SUMOCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `opencode serve` og `opencode web`.

```bash
SUMOCODE_SERVER_PASSWORD=your-password opencode serve
```
...
SumoCode TUI kjører, vil `opencode serve` starte en ny server.

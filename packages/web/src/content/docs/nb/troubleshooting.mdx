...
For å feilsøke problemer med SumoCode, start med å sjekke loggene og lokale data den lagrer på disken.
...
SumoCode lagrer øktdata og andre applikasjonsdata på disken på:
...
SumoCode Desktop kjører en lokal SumoCode-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `SumoCode`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/sumocode/sumocode.jsonc` (eller `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at SumoCode kan gjenoppbygge den.

1. Avslutt SumoCode Desktop helt.
...
3. Start SumoCode Desktop på nytt.

---

### Løs problemer med servertilkobling

SumoCode Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis `sumocode.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `SUMOCODE_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `SUMOCODE_PORT` (eller velg en ledig port) og start på nytt.
...
På Windows krever SumoCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til SumoCode.

---

### Varsler vises ikke

SumoCode Desktop viser bare systemvarsler når:

- varsler er aktivert for SumoCode i OS-innstillingene dine, og
...
1. Avslutt SumoCode Desktop.
2. Finn og slett disse filene (de finnes i SumoCode Desktop-appens datakatalog):
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. SumoCode installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start SumoCode på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge SumoCode til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
SumoCode vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.

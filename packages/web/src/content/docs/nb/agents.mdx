...
Det er to typer agenter i SumoCode; primære agenter og underagenter.
...
SumoCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
SumoCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

SumoCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `sumocode.json`:
...
Hvis ingen temperatur er spesifisert, bruker SumoCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale SumoCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i SumoCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [SumoCode Zen](/docs/zen), vil du bruke `sumocode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `sumocode models` for å se en liste over tilgjengelige modeller.
:::

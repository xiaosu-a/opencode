...
description: Lag verktøy som LLM kan kalle inn SumoCode.
---

Egendefinerte verktøy er funksjoner du oppretter som LLM kan kalle under samtaler. De fungerer sammen med SumoCode sine [innebygde verktøy](/docs/tools) som `read`, `write` og `bash`.
...
- Lokalt ved å plassere dem i `.sumocode/tools/`-katalogen til prosjektet ditt.
- Eller globalt, ved å plassere dem i `~/.config/sumocode/tools/`.
...
```ts title=".sumocode/tools/bash.ts"
import { tool } from "@sumocode-ai/plugin"

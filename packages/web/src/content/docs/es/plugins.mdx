---
title: Complementos
description: Escriba sus propios complementos para extender SumoCode.
---

Los complementos le permiten extender SumoCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de SumoCode.
...
- `.sumocode/plugins/` - Complementos a nivel de proyecto
- `~/.config/sumocode/plugins/` - Complementos globales
...
1. Configuración global (`~/.config/sumocode/sumocode.json`)
2. Configuración del proyecto (`sumocode.json`)
3. Directorio global de complementos (`~/.config/sumocode/plugins/`)
4. Directorio de complementos del proyecto (`.sumocode/plugins/`)
...
SumoCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
:::note
Si está utilizando la aplicación de escritorio SumoCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://sumocode.ai/docs/sdk) para obtener más detalles.

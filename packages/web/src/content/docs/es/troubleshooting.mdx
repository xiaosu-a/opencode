...
Para depurar problemas con SumoCode, comience verificando los registros y los datos locales que almacena en el disco.
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `sumocode --log-level DEBUG`.

---

## Almacenamiento

sumocode almacena datos de sesión y otros datos de aplicaciones en el disco en:
...
SumoCode Desktop ejecuta un servidor SumoCode local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `SumoCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/sumocode/sumocode.jsonc` (o `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que SumoCode pueda reconstruirlo.

1. Salga de SumoCode Desktop por completo.
...
3. Reinicie el escritorio SumoCode.

---

### Solucionar problemas de conexión del servidor

SumoCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `sumocode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio SumoCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de SumoCode.

---

### Notificaciones que no se muestran

SumoCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para SumoCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio SumoCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio SumoCode):
...
Si tiene problemas con SumoCode:
...
3. Asegúrese de tener la última versión con `sumocode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `sumocode models`
...
sumocode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.

---
title: Proveedores
description: Usando cualquier proveedor LLM en SumoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SumoCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración SumoCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu SumoCode
configuración.
...
SumoCode Zen es una lista de modelos proporcionados por el equipo SumoCode que han sido
probado y verificado para funcionar bien con SumoCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con SumoCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [sumocode.ai/auth](https://sumocode.ai/auth).
...
Funciona como cualquier otro proveedor en SumoCode y su uso es completamente opcional.

---

## SumoCode Go

SumoCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de SumoCode que han sido
probado y verificado para funcionar bien con SumoCode.

1. Ejecute el comando `/connect` en TUI, seleccione `SumoCode Go` y diríjase a [sumocode.ai/auth](https://sumocode.ai/zen).
...
Para usar Amazon Bedrock con SumoCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `sumocode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en SumoCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `SumoCode`, Alcances: `api`
...
SumoCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear SumoCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `sumocode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `sumocode.json`:
...
Para utilizar Google Vertex AI con SumoCode:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación SumoCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación SumoCode enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para SumoCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con SumoCode:
...
3. Copie la clave API para usarla en SumoCode.
...
6. **Importante**: Antes de usar modelos en la nube en SumoCode, debe obtener la información del modelo localmente:
...
SumoCode Zen es una lista de modelos probados y verificados proporcionada por el equipo SumoCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>SumoCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **SumoCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave SumoCode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con SumoCode:
...
4. Cree o actualice su archivo `sumocode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a SumoCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.

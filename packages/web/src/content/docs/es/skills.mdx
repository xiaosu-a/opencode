...
Las habilidades del agente permiten a SumoCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
SumoCode busca estas ubicaciones:

- Configuración del proyecto: `.sumocode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/sumocode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, SumoCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.sumocode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.sumocode/skills/git-release/SKILL.md` así:
...
SumoCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `sumocode.json`:
...
**Para agentes integrados** (en `sumocode.json`):

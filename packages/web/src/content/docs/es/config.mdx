---
title: Configuración
description: Usando la configuración SumoCode JSON.
---

Puede configurar SumoCode usando un archivo de configuración JSON.

---

## Formato

SumoCode admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
2. **Configuración global** (`~/.config/sumocode/sumocode.json`) - preferencias del usuario
3. **Configuración personalizada** (`SUMOCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`sumocode.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.opencode`** - agentes, comandos, complementos
6. **Configuración en línea** (`SUMOCODE_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.opencode` y `~/.config/sumocode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global SumoCode en `~/.config/sumocode/sumocode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `sumocode.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia SumoCode, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `SUMOCODE_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `SUMOCODE_CONFIG_DIR`
...
El archivo de configuración tiene un esquema definido en [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor SumoCode.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración SumoCode a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, SumoCode intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración SumoCode a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/sumocode/agents/` o `.sumocode/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, SumoCode recurrirá a `"build"` con una advertencia.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/sumocode/commands/` o `.sumocode/commands/`. [Más información aquí](/docs/commands).
...
SumoCode descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían SumoCode con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.sumocode/plugins/` o `~/.config/sumocode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir SumoCode para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.

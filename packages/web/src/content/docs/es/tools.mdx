...
Las herramientas permiten que LLM realice acciones en su código base. SumoCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en SumoCode.
...
:::note
Esta herramienta solo está disponible cuando `SUMOCODE_EXPERIMENTAL_LSP_TOOL=true` (o `SUMOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor SumoCode o cuando la variable de entorno `SUMOCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar SumoCode:

```bash
SUMOCODE_ENABLE_EXA=1 opencode
```

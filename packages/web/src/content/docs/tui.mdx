---
title: TUI
description: Using the SumoCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

SumoCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running SumoCode starts the TUI for the current directory.
...
When using the SumoCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to SumoCode. Allows you to select from available providers and add their API keys.
...
Exit SumoCode. _Aliases_: `/quit`, `/q`
...
This is separate from `sumocode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long SumoCode waits after the leader key. Defaults to `2000`.
...
Use `SUMOCODE_TUI_CONFIG` to load a custom TUI config path.

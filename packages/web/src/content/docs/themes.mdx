...
With SumoCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, SumoCode uses our own `opencode` theme.
...
SumoCode comes with several built-in themes.
...
- Want SumoCode to match their terminal's appearance
...
SumoCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/sumocode/themes/*.json` or `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.sumocode/themes/*.json`
4. **Current working directory** - Defined in `./.sumocode/themes/*.json`

...
Komanda `sumocode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opencode klijent moze koristiti.
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `opencode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `sumocode serve` i za `sumocode web`.
...
Mozete pokrenuti `sumocode serve` da startate standalone server. Ako je opencode TUI vec pokrenut, `sumocode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste SumoCode [IDE](/docs/ide) pluginovi.
...
sumocode server izlaže sljedece API-je.

...
Da biste otklonili probleme s SumoCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `sumocode --log-level DEBUG`.

---

## Pohrana

SumoCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
SumoCode Desktop pokreće lokalni SumoCode server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `SumoCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/sumocode/sumocode.jsonc` (ili `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/sumocode/plugins/`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga SumoCode mogao ponovo izgraditi.

1. Potpuno zatvorite SumoCode Desktop.
...
3. Ponovo pokrenite SumoCode Desktop.

---

### Rješavanje problema sa vezom na serveru

SumoCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `sumocode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, SumoCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa SumoCode karakteristikama.

---

### Obavještenja se ne prikazuju

SumoCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za SumoCode u postavkama vašeg OS-a, i
...
1. Zatvorite SumoCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka SumoCode Desktop aplikacije):
...
Da saznate kojim modelima imate pristup, pokrenite `sumocode models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. SumoCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite SumoCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti SumoCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
SumoCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

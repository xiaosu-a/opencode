...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. SumoCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u SumoCode.
...
:::note
Ovaj alat je dostupan samo kada je `SUMOCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `SUMOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz SumoCode provajdera ili kada je varijabla `SUMOCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju SumoCode:

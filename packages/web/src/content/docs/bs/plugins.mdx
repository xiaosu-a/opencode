---
title: Dodaci
description: Napišite vlastite dodatke za proširenje SumoCode.
---

Dodaci vam omogućavaju da proširite SumoCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja SumoCode.
...
- `.sumocode/plugins/` - Dodaci na nivou projekta
- `~/.config/sumocode/plugins/` - Globalni dodaci
...
1. Globalna konfiguracija (`~/.config/sumocode/sumocode.json`)
2. Konfiguracija projekta (`sumocode.json`)
3. Globalni direktorij dodataka (`~/.config/sumocode/plugins/`)
4. Direktorij dodataka projekta (`.sumocode/plugins/`)
...
SumoCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje SumoCode.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://sumocode.ai/docs/sdk) za detalje.

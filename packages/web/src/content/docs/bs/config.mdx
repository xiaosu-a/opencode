---
title: Konfiguracija
description: Korištenje SumoCode JSON konfiguracije.
---

Možete konfigurirati SumoCode koristeći JSON konfiguracijski fajl.

---

## Format

SumoCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/sumocode/sumocode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SUMOCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`sumocode.json` u projektu) - postavke specifične za projekat
5. **`.opencode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SUMOCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.opencode` i `~/.config/sumocode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu SumoCode konfiguraciju u `~/.config/sumocode/sumocode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/sumocode/tui.json`.
...
Dodajte `sumocode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se SumoCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SUMOCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SUMOCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).

TUI konfiguracija koristi [**`sumocode.ai/tui.json`**](https://sumocode.ai/tui.json).
...
Koristite `SUMOCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `sumocode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš SumoCode server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj SumoCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, SumoCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/sumocode/agents/` ili `.sumocode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, SumoCode će se vratiti na `"build"` s upozorenjem.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/sumocode/commands/` ili `.sumocode/commands/`. [Saznajte više ovdje](/docs/commands).
...
SumoCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, SumoCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju SumoCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.sumocode/plugins/` ili `~/.config/sumocode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite SumoCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

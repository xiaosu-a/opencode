...
Možete dodati vanjske alate u SumoCode koristeći _Model Context Protocol_, ili MCP. SumoCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [SumoCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
SumoCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, SumoCode će:
...
Ako server zahtijeva autentifikaciju, SumoCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `sumocode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, SumoCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u SumoCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz SumoCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete SumoCode sa vašim Sentry nalogom.

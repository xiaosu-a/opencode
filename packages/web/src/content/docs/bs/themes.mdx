...
U SumoCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, SumoCode koristi nasu `opencode` temu.
...
SumoCode dolazi sa vise ugradenih tema.
...
- Zele da SumoCode odgovara izgledu njihovog terminala
...
SumoCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/sumocode/themes/*.json` ili `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.sumocode/themes/*.json`
4. **Trenutni radni direktorij** - `./.sumocode/themes/*.json`

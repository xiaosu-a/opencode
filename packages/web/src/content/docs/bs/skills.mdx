...
Agent skills omogucavaju SumoCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
SumoCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.sumocode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/sumocode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, SumoCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.sumocode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.sumocode/skills/git-release/SKILL.md` ovako:
...
SumoCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `sumocode.json`:
...
**Za ugradene agente** (u `sumocode.json`):

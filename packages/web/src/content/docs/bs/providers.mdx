---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u SumoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SumoCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj SumoCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem SumoCode
config.
...
SumoCode Zen je lista modela koje je obezbedio SumoCode tim koji su bili
testirano i potvrđeno da dobro radi sa SumoCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa SumoCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite opencode i idite na [sumocode.ai/auth](https://sumocode.ai/auth).
...
Radi kao i svaki drugi provajder u SumoCode i potpuno je opcionalan za korištenje.

---

## SumoCode Go

SumoCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža SumoCode tim i koji su testirani i verificirani da dobro rade s SumoCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `SumoCode Go` i idite na [sumocode.ai/auth](https://sumocode.ai/zen).
...
Da biste koristili Amazon Bedrock s SumoCode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `sumocode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u SumoCode.
:::
...
SumoCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate SumoCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`sumocode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `sumocode.json`:
...
Za korištenje Google Vertex AI s SumoCode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki SumoCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki SumoCode razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za SumoCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s SumoCode:
...
3. Kopirajte API ključ za korištenje u SumoCode.
...
6. **Važno**: Prije upotrebe modela oblaka u SumoCode, morate lokalno povući informacije o modelu:
...
SumoCode Zen je lista testiranih i verifikovanih modela koju je obezbedio SumoCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>SumoCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **SumoCode Zen**.

```txt
   /connect
```

3. Unesite svoj SumoCode API ključ.
...
4. Kreirajte ili ažurirajte svoju `sumocode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju SumoCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite SumoCode konfiguraciju i:

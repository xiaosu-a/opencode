...
`sumocode serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 opencode 客戶端使用。
...
設定 `OPENCODE_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `opencode`，也可以設定 `OPENCODE_SERVER_USERNAME` 來覆蓋它。這適用於 `sumocode serve` 和 `sumocode web`。
...
你可以執行 `sumocode serve` 來啟動一個獨立的伺服器。如果你已經在執行 opencode TUI，`sumocode serve` 會啟動一個新的伺服器。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 SumoCode [IDE](/docs/ide) 外掛所使用。

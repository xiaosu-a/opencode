---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um SumoCode zu erweitern.
---

Mit Plugins können Sie SumoCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von SumoCode zu ändern.
...
- `.sumocode/plugins/` – Plugins auf Projektebene
- `~/.config/sumocode/plugins/` – Globale Plugins
...
1. Globale Konfiguration (`~/.config/sumocode/sumocode.json`)
2. Projektkonfiguration (`sumocode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/sumocode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.sumocode/plugins/`)
...
SumoCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein SumoCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie SumoCode erweitern können.
...
:::note
Wenn Sie die SumoCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass SumoCode `.env`-Dateien liest:
...
Plugins können SumoCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das SumoCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für SumoCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://sumocode.ai/docs/sdk).

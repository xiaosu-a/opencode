---
title: Konfiguration
description: Verwenden Sie die Konfiguration SumoCode JSON.
---

Sie können SumoCode mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

SumoCode unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
2. **Globale Konfiguration** (`~/.config/sumocode/sumocode.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`SUMOCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`sumocode.json` im Projekt) – projektspezifische Einstellungen
5. **`.opencode` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`SUMOCODE_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.opencode` und `~/.config/sumocode` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale SumoCode-Konfiguration in `~/.config/sumocode/sumocode.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `sumocode.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn SumoCode gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `SUMOCODE_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `SUMOCODE_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json) definiert ist.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres SumoCode-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer SumoCode-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht SumoCode, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer SumoCode-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/sumocode/agents/` oder `.sumocode/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift SumoCode mit einer Warnung auf `"build"` zurück.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/sumocode/commands/` oder `.sumocode/commands/` definieren. [Mehr erfahren](/docs/commands).
...
SumoCode lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
SumoCode erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert SumoCode mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.sumocode/plugins/` oder `~/.config/sumocode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie SumoCode darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.

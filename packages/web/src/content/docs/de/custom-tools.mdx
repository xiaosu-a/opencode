...
description: Erstellen Sie Tools, die der LLM in SumoCode aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von SumoCode wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.sumocode/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/sumocode/tools/` platzieren.
...
```ts title=".sumocode/tools/database.ts" {1}
...
```ts title=".sumocode/tools/math.ts"
import { tool } from "@sumocode-ai/plugin"
...
```ts title=".sumocode/tools/project.ts" {8}
import { tool } from "@sumocode-ai/plugin"
...
```python title=".sumocode/tools/add.py"
import sys
...
```ts title=".sumocode/tools/python-add.ts" {10}
import { tool } from "@sumocode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".sumocode/tools/add.py")

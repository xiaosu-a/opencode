...
Agent Skills erlauben SumoCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
SumoCode sucht in folgenden Pfaden:

- Project config: `.sumocode/skills/<name>/SKILL.md`
- Global config: `~/.config/sumocode/skills/<name>/SKILL.md`
...
Bei projektlokalen Pfaden laeuft SumoCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.sumocode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.sumocode/skills/git-release/SKILL.md` zum Beispiel so:
...
SumoCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `sumocode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `sumocode.json`):

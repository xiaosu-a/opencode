...
Wenn SumoCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `sumocode --log-level DEBUG`.

---

## Speicher

sumocode speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
SumoCode Desktop startet im Hintergrund einen lokalen SumoCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `SumoCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/sumocode/sumocode.jsonc` (oder `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit SumoCode ihn neu aufbauen kann.

1. Beende SumoCode Desktop komplett.
...
3. Starte SumoCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

SumoCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `sumocode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt SumoCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit SumoCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

SumoCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer SumoCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende SumoCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von SumoCode Desktop):
...
Wenn du Probleme mit SumoCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `sumocode models` aus.
...
sumocode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).

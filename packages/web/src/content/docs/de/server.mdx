...
Der Befehl `sumocode serve` startet einen headless HTTP-Server.
...
Der Benutzername ist standardmaessig `opencode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `sumocode serve` und `sumocode web`.
...
Mit `sumocode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `sumocode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die SumoCode-[IDE](/docs/ide)-Plugins.

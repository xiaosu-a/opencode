...
Es gibt zwei Arten von Agenten in SumoCode; Primäragenten und Subagenten.
...
SumoCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `sumocode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet SumoCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale SumoCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer SumoCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [SumoCode Zen](/docs/zen) verwenden, würden Sie `sumocode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `sumocode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::

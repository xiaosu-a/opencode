...
Du kannst auch globale Regeln in `~/.config/sumocode/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt SumoCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/sumocode/AGENTS.md` existiert)
...
2. **Globale Datei** unter `~/.config/sumocode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `sumocode.json` oder in `~/.config/sumocode/sumocode.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `sumocode.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `sumocode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

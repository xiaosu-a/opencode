---
title: Config
description: Using the SumoCode JSON config.
---

You can configure SumoCode using a JSON config file.

---

## Format

SumoCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="sumocode.jsonc"
{
...
2. **Global config** (`~/.config/sumocode/sumocode.json`) - user preferences
3. **Custom config** (`SUMOCODE_CONFIG` env var) - custom overrides
4. **Project config** (`sumocode.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`SUMOCODE_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.opencode` and `~/.config/sumocode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global SumoCode config in `~/.config/sumocode/sumocode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/sumocode/tui.json`.
...
Add `sumocode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When SumoCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `SUMOCODE_CONFIG` environment variable.

```bash
export SUMOCODE_CONFIG=/path/to/my/custom-config.json
...
Specify a custom config directory using the `SUMOCODE_CONFIG_DIR`
...
```bash
export SUMOCODE_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```
...
Drop an `sumocode.json` or `sumocode.jsonc` file in the system managed config directory:
...
On macOS, SumoCode reads managed preferences from the `ai.sumocode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

SumoCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.sumocode.managed.plist`
2. `/Library/Managed Preferences/ai.sumocode.managed.plist`

The plist keys map directly to `sumocode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.sumocode.managed` PayloadType. The SumoCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.sumocode.managed</string>
...
The server/runtime config schema is defined in [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).

TUI config uses [**`sumocode.ai/tui.json`**](https://sumocode.ai/tui.json).
...
Use `SUMOCODE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `sumocode.json` are deprecated and automatically migrated when possible.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your SumoCode server.
...
If not specified, SumoCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your SumoCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, SumoCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny SumoCode actions on configured resources. Currently, policies can control which providers SumoCode may use.
...
SumoCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, SumoCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/sumocode/agents/` or `.sumocode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, SumoCode will fall back to `"build"` with a warning.
...
You can also define commands using markdown files in `~/.config/sumocode/commands/` or `.sumocode/commands/`. [Learn more here](/docs/commands).
...
SumoCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
SumoCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend SumoCode with custom tools, hooks, and integrations.

Place plugin files in `.sumocode/plugins/` or `~/.config/sumocode/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict SumoCode to only use specific providers rather than disabling them one by one.

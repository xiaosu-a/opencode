...
On macOS, SumoCode reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.
...
Use the `ai.opencode.managed` PayloadType. The SumoCode config keys go directly in the payload dict:
...
You can configure server settings for the `sumocode serve` and `sumocode web` commands through the `server` option.
...
This setting applies across all interfaces: TUI, CLI (`sumocode run`), desktop app, and GitHub Action.

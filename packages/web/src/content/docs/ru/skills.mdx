...
sumocode выполняет поиск в следующих местах:

- Конфигурация проекта: `.sumocode/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/sumocode/skills/<name>/SKILL.md`.
...
Он загружает все соответствующие `skills/*/SKILL.md` в `.sumocode/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.sumocode/skills/git-release/SKILL.md` следующим образом:
...
sumocode перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `sumocode.json`:
...
**Для встроенных агентов** (в формате `sumocode.json`):

...
description: Usa la configurazione JSON di SumoCode.
---

Puoi configurare SumoCode usando un file di configurazione JSON.

---

## Formato

SumoCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/sumocode/sumocode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `SUMOCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`sumocode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.opencode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `SUMOCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.opencode` e `~/.config/sumocode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di SumoCode in `~/.config/sumocode/sumocode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/sumocode/tui.json`.
...
Aggiungi `sumocode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando SumoCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `SUMOCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `SUMOCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).

La configurazione TUI usa [**`sumocode.ai/tui.json`**](https://sumocode.ai/tui.json).
...
Usa `SUMOCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `sumocode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server SumoCode.
...
Puoi configurare provider e modelli da usare in SumoCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, SumoCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/sumocode/agents/` o `.sumocode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, SumoCode fara fallback su `"build"` mostrando un avviso.
...
Puoi anche definire comandi usando file markdown in `~/.config/sumocode/commands/` o `.sumocode/commands/`. [Scopri di piu](/docs/commands).
...
SumoCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono SumoCode con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.sumocode/plugins/` o `~/.config/sumocode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare SumoCode a usare solo alcuni provider invece di disabilitarli uno per uno.

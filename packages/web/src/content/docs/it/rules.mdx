...
Puoi anche avere regole globali in un file `~/.config/sumocode/AGENTS.md`. Queste si applicano a tutte le sessioni di opencode.
...
Per gli utenti che migrano da Claude Code, SumoCode supporta come fallback le convenzioni di file di Claude Code:

- **Regole di progetto**: `CLAUDE.md` nella directory del progetto (usato se non esiste `AGENTS.md`)
- **Regole globali**: `~/.claude/CLAUDE.md` (usato se non esiste `~/.config/sumocode/AGENTS.md`)
...
2. **File globale** in `~/.config/sumocode/AGENTS.md`
3. **File Claude Code** in `~/.claude/CLAUDE.md` (se non disabilitato)

In ogni categoria vince il primo file corrispondente. Per esempio, se hai sia `AGENTS.md` sia `CLAUDE.md`, viene usato solo `AGENTS.md`. Allo stesso modo, `~/.config/sumocode/AGENTS.md` ha priorita' su `~/.claude/CLAUDE.md`.

---

## Istruzioni personalizzate

Puoi specificare file di istruzioni personalizzati nel tuo `sumocode.json` o nel globale `~/.config/sumocode/sumocode.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
L'approccio consigliato e' usare il campo `instructions` in `sumocode.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `sumocode.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::

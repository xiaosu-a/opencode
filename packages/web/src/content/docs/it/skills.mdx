...
Le skill degli agenti permettono a SumoCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
SumoCode cerca in queste posizioni:

- Config di progetto: `.sumocode/skills/<name>/SKILL.md`
- Config globale: `~/.config/sumocode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, SumoCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.sumocode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.sumocode/skills/git-release/SKILL.md` cosi':
...
SumoCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `sumocode.json`:
...
**Per agenti integrati** (in `sumocode.json`):

---
title: Plugin
description: Scrivi plugin per estendere SumoCode.
---

I plugin ti permettono di estendere SumoCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di SumoCode.
...
- `.sumocode/plugins/` - plugin a livello progetto
- `~/.config/sumocode/plugins/` - plugin globali
...
1. Config globale (`~/.config/sumocode/sumocode.json`)
2. Config di progetto (`sumocode.json`)
3. Directory plugin globale (`~/.config/sumocode/plugins/`)
4. Directory plugin di progetto (`.sumocode/plugins/`)
...
SumoCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di SumoCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://sumocode.ai/docs/sdk) per i dettagli.

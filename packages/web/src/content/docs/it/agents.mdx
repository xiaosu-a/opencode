...
In SumoCode esistono due tipi di agenti: agenti primari e subagenti.
...
SumoCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
SumoCode include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

SumoCode include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di SumoCode, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `sumocode.json`:
...
- Globale: `~/.config/sumocode/agents/`
- Per progetto: `.sumocode/agents/`
...
Se non specifichi una temperature, SumoCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di SumoCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione SumoCode usa il formato `provider/model-id`. Per esempio, se stai usando [SumoCode Zen](/docs/zen), useresti `sumocode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `sumocode models` per vedere la lista dei modelli disponibili.
:::

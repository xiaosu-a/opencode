---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di SumoCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

SumoCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire SumoCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di SumoCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a SumoCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da SumoCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `sumocode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo SumoCode attende dopo il leader key. Default: `2000`.

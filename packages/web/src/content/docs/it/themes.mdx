...
Con SumoCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, SumoCode usa il tema `opencode`.
...
SumoCode include diversi temi integrati.
...
- Vuole che SumoCode corrisponda all'aspetto del terminale
...
SumoCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/sumocode/themes/*.json` o `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.sumocode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.sumocode/themes/*.json`

...
SumoCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `sumocode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.
...
Puoi anche collegarti a una istanza `sumocode serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
Avvia un server HTTP e apre un browser per accedere a SumoCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).

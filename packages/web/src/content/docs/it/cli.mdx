---
title: CLI
description: Opzioni e comandi della CLI di SumoCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di SumoCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con SumoCode in modo programmatico.
...
Avvia l'interfaccia testuale di SumoCode (interfaccia utente da terminale).
...
La CLI di SumoCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per SumoCode.
...
Collega un terminale a un backend server di SumoCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend SumoCode remoto. Per esempio:
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `SUMOCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `SUMOCODE_SERVER_USERNAME` o `opencode`) |
...
SumoCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando SumoCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in SumoCode.
...
Avvia un server SumoCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
opencode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `SUMOCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Elenca tutte le sessioni SumoCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni SumoCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione SumoCode.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione SumoCode.
...
Avvia un server SumoCode headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a SumoCode tramite interfaccia web. Imposta `SUMOCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla SumoCode e rimuove tutti i file correlati.
...
SumoCode può essere configurato tramite variabili d'ambiente.

| Variabile                             | Tipo    | Descrizione                                                 |
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `SUMOCODE_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `SUMOCODE_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `SUMOCODE_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `SUMOCODE_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `SUMOCODE_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `SUMOCODE_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `SUMOCODE_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `SUMOCODE_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `SUMOCODE_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `SUMOCODE_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `SUMOCODE_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `SUMOCODE_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `SUMOCODE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `SUMOCODE_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `SUMOCODE_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `SUMOCODE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `SUMOCODE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `SUMOCODE_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `SUMOCODE_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `SUMOCODE_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `SUMOCODE_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `SUMOCODE_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `SUMOCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `SUMOCODE_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `SUMOCODE_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `SUMOCODE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `SUMOCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `SUMOCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `SUMOCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `SUMOCODE_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `SUMOCODE_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `SUMOCODE_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `SUMOCODE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `SUMOCODE_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `SUMOCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `SUMOCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `SUMOCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `SUMOCODE_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `SUMOCODE_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `SUMOCODE_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `SUMOCODE_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `SUMOCODE_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |

---
title: Server LSP
description: SumoCode si integra con i tuoi server LSP.
---

SumoCode può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

SumoCode include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `SUMOCODE_DISABLE_LSP_DOWNLOAD` a `true`.
:::

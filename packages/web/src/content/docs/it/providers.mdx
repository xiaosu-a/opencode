---
title: Provider
description: Usa qualsiasi provider LLM in SumoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SumoCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di SumoCode.
...
SumoCode Zen e una lista di modelli forniti dal team SumoCode, testati e
verificati per funzionare bene con SumoCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da SumoCode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opencode e vai su [sumocode.ai/auth](https://sumocode.ai/auth).
...
Funziona come qualsiasi altro provider in SumoCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con SumoCode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `sumocode.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in SumoCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
SumoCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare SumoCode
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `sumocode.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `sumocode.json`:
...
Per usare Google Vertex AI con SumoCode:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di SumoCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di SumoCode elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per SumoCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con SumoCode:
...
3. Copia la chiave API da usare in SumoCode.
...
6. **Importante**: Prima di usare modelli cloud in SumoCode, devi scaricare le informazioni del modello localmente:
...
SumoCode Zen è una lista di modelli testati e verificati forniti dal team SumoCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>SumoCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **SumoCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di SumoCode.
...
4. Crea o aggiorna il tuo file `sumocode.json` nella directory del progetto:
...
I campi `limit` permettono a SumoCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.

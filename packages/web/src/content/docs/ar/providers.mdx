---
title: المزوّدون
description: استخدام أي موفّر LLM في SumoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم SumoCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات SumoCode.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات SumoCode.
...
SumoCode Zen هي قائمة نماذج يوفّرها فريق SumoCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع SumoCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع SumoCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [sumocode.ai/auth](https://sumocode.ai/auth).
...
يعمل مثل أي مزوّد آخر في SumoCode واستخدامه اختياري بالكامل.

---

## SumoCode Go

SumoCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق SumoCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع SumoCode.

1. شغّل الأمر `/connect` في TUI، واختر `SumoCode Go`، ثم انتقل إلى [sumocode.ai/auth](https://sumocode.ai/zen).
...
لاستخدام Amazon Bedrock مع SumoCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `sumocode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل SumoCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم SumoCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر SumoCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`sumocode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام Google Vertex AI مع SumoCode:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة SumoCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة SumoCode مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع SumoCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع SumoCode:
...
3. انسخ مفتاح API لاستخدامه في SumoCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في SumoCode، يجب سحب معلومات النموذج محليا:
...
SumoCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق SumoCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>SumoCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **SumoCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ SumoCode.
...
4. أنشئ أو حدّث ملف `sumocode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ SumoCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.

...
تتيح مهارات الوكيل لـ SumoCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث SumoCode في هذه المواقع:

- إعدادات المشروع: `.sumocode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/sumocode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج SumoCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.sumocode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/sumocode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.sumocode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض SumoCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `sumocode.json`:
...
**للوكلاء المدمجين** (في `sumocode.json`):

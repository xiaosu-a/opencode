---
title: プラグイン
description: SumoCode を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで SumoCode を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、SumoCode のデフォルトの動作を変更したりできます。
...
- `.sumocode/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/sumocode/plugins/` - グローバルプラグイン
...
1. グローバル設定 (`~/.config/sumocode/sumocode.json`)
2. プロジェクト設定 (`sumocode.json`)
3. グローバルプラグインディレクトリ (`~/.config/sumocode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.sumocode/plugins/`)
...
SumoCode は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための SumoCode SDK クライアント。
...
ここでは、SumoCode を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://sumocode.ai/docs/sdk) を参照してください。
...
SumoCode が `.env` ファイルを読み取らないようにします。
...
プラグインは SumoCode にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、SumoCode が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して SumoCode で使用できます。

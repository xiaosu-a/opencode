---
title: サーバー
description: HTTP 経由で SumoCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`sumocode serve` コマンドは、SumoCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `opencode` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`sumocode serve` と `sumocode web` の両方に当てはまります。
...
:::tip
SumoCode サーバーを使用して、プログラムで SumoCode と対話します。
:::

`sumocode serve` を実行してスタンドアロンサーバーを起動できます。すでに SumoCode TUI を実行している場合でも、`sumocode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、SumoCode [IDE](/docs/ide) プラグインによって使用されます。

...
Med SumoCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger SumoCode vårt eget `opencode`-tema.
...
SumoCode kommer med flere innebygde temaer.
...
- Vil at SumoCode skal matche terminalens utseende
...
SumoCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/sumocode/themes/*.json` eller `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.sumocode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.sumocode/themes/*.json`

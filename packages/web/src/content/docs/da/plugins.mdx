---
title: Plugins
description: Skriv dine egne plugins for at udvide SumoCode.
---

Plugins giver dig mulighed for at udvide SumoCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre SumoCodes standardadfærd.
...
- `.sumocode/plugins/` - Plugins på projektniveau
...
1. Global konfiguration (`~/.config/sumocode/sumocode.json`)
2. Projektkonfiguration (`sumocode.json`)
3. Global plugin-mappe (`~/.config/sumocode/plugins/`)
4. Projekt plugin bibliotek (`.sumocode/plugins/`)
...
SumoCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
:::note
Hvis du bruger SumoCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://sumocode.ai/docs/sdk) for detaljer.

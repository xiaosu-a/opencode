...
For at fejlfinde problemer med SumoCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `sumocode --log-level DEBUG`.

---

## Lagring

sumocode gemmer sessionsdata og andre applikationsdata på disken på:
...
SumoCode Desktop kører en lokal SumoCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `SumoCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så SumoCode kan genopbygge den.

1. Afslut SumoCode Desktop helt.
2. Slet cache-mappen:
...
3. Start SumoCode Desktop på ny.

---

### Løs problemer med serverforbindelse

SumoCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `sumocode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver SumoCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i SumoCode.

---

### Meddelelser vises ikke

SumoCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for SumoCode i dine OS-indstillinger, og
...
1. Afslut SumoCode Desktop.
2. Find og slet disse filer (de findes i SumoCode Desktop-appens datamappe):
...
Hvis du oplever problemer med SumoCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `sumocode models`
...
sumocode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.

...
Her er alle de innebygde verktøyene som er tilgængelige i SumoCode.
...
:::note
Dette verktøyet er bare tilgængelig når `SUMOCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `SUMOCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger SumoCode-leverandøren eller når miljøvariabelen `SUMOCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

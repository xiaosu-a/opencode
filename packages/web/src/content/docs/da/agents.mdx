...
Der er to typer agenter i SumoCode; primære agenter og subagenter.
...
SumoCode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
SumoCode leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

SumoCode leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i SumoCode's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `sumocode.json`-konfigurationsfil:
...
- Globalt: `~/.config/sumocode/agents/`
- Pr. projekt: `.sumocode/agents/`
...
Hvis ingen temperatur er angivet, bruger SumoCode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale SumoCode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din SumoCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [SumoCode Zen](/docs/zen), vil du bruge `sumocode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `sumocode models` for at se en liste over de tilgængelige modeller.
:::

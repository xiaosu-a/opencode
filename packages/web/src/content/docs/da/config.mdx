---
title: Konfiguration
description: Ved at bruge SumoCode JSON konfig.
---

Du kan konfigurere SumoCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

SumoCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/sumocode/sumocode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`SUMOCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`sumocode.json` i projekt) - projektspecifikke indstillinger
5. **`.opencode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`SUMOCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.opencode` og `~/.config/sumocode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale SumoCode-konfiguration i `~/.config/sumocode/sumocode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `sumocode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når SumoCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `SUMOCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `SUMOCODE_CONFIG_DIR`
...
Konfigurationsfilen har et skema, der defineres i [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din SumoCode-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din SumoCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger SumoCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din SumoCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/sumocode/agents/` eller `.sumocode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil SumoCode falde tilbage til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/sumocode/commands/` eller `.sumocode/commands/`. [Learn more here](/docs/commands).
...
SumoCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide SumoCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.sumocode/plugins/` eller `~/.config/sumocode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse SumoCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.

...
Agentferdigheter lar SumoCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
SumoCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.sumocode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/sumocode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går SumoCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.sumocode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.sumocode/skills/git-release/SKILL.md` slik:
...
SumoCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `sumocode.json`:
...
**For innebygde agenter** (i `sumocode.json`):

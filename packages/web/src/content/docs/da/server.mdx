...
Kommandoen `sumocode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `opencode`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `sumocode serve` og `sumocode web`.
...
Du kan køre `sumocode serve` for at starte en selvstændig server. Hvis du har
sumocode TUI kørende, vil `sumocode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af SumoCode [IDE](/docs/ide) plugins.
...
SumoCode-serveren viser følgende API'er.

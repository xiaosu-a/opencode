---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i SumoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SumoCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i SumoCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i SumoCode-konfigurationen.
...
SumoCode Zen er en liste over modeller leveret af SumoCode-teamet, som er blevet testet og verificeret til at fungere godt med SumoCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med SumoCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opencode og gå til [sumocode.ai/auth](https://sumocode.ai/auth).
...
Det fungerer som alle andre udbydere i SumoCode og er helt valgfrit at bruge.

---

## SumoCode Go

SumoCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af SumoCode-teamet, som er testet og verificeret til at fungere godt med SumoCode.

1. Kør kommandoen `/connect` i TUI, vælg `SumoCode Go`, og gå til [sumocode.ai/auth](https://sumocode.ai/zen).
...
Sådan bruger du Amazon Bedrock med SumoCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `sumocode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i SumoCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
SumoCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse SumoCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`sumocode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med SumoCode:
...
Sådan bruger du Google Vertex AI med SumoCode:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver SumoCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver SumoCode-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for SumoCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med SumoCode:
...
3. Kopier API-nøglen til brug i SumoCode.
...
6. **Vigtigt**: Før du bruger skymodeller i SumoCode, skal du hente modelinformationen lokalt:
...
SumoCode Zen er en liste over testede og verificerede modeller leveret af SumoCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>SumoCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **SumoCode Zen**.

   ```txt
   /connect
   ```

3. Indtast SumoCode API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med SumoCode:
...
4. Opret eller opdater `sumocode.json`-filen i projektkataloget:
...
`limit`-felterne lader SumoCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.

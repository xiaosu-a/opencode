...
To debug issues with SumoCode, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `sumocode --log-level DEBUG`.

---

## Storage

sumocode stores session data and other application data on disk at:
...
To uninstall the SumoCode CLI and remove its related files, run:
...
To remove SumoCode Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

SumoCode Desktop runs a local SumoCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `SumoCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/sumocode/sumocode.jsonc` (or `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so SumoCode can rebuild it.

1. Quit SumoCode Desktop completely.
...
3. Restart SumoCode Desktop.

---

### Fix server connection issues

SumoCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `sumocode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, SumoCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with SumoCode's features.

---

### Notifications not showing

SumoCode Desktop only shows system notifications when:

- notifications are enabled for SumoCode in your OS settings, and
...
1. Quit SumoCode Desktop.
2. Find and delete these files (they live in the SumoCode Desktop app data directory):
...
If you're experiencing issues with SumoCode:
...
### SumoCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `sumocode upgrade`
...
To figure out what models you have access to, run `sumocode models`
...
sumocode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.

...
You can add external tools to SumoCode using the _Model Context Protocol_, or MCP. SumoCode supports both local and remote servers.
...
You can define MCP servers in your [SumoCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
SumoCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, SumoCode will:
...
If the server requires authentication, SumoCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `sumocode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, SumoCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in SumoCode, alongside built-in tools. So you can manage them through the SumoCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect SumoCode to your Sentry account.

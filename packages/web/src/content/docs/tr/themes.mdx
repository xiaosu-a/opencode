...
sumocode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.
...
sumocode birden fazla yerleşik temayla gelir.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/sumocode/themes/*.json` veya `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.sumocode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.sumocode/themes/*.json`

...
:::caution
`SUMOCODE_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::
...
Erişimi korumak için `SUMOCODE_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
SUMOCODE_SERVER_PASSWORD=secret opencode web
```

Kullanıcı adı varsayılan olarak `opencode` şeklindedir ancak `SUMOCODE_SERVER_USERNAME` ile değiştirilebilir.

...
En iyi deneyim için PowerShell yerine `sumocode web`'yi [WSL](/docs/windows-wsl)'den çalıştırın. Bu, uygun dosya sistemi erişimini ve terminal entegrasyonunu sağlar.
:::
...
sumocode varsayılan olarak kullanılabilir bir bağlantı noktasını seçer. Bir bağlantı noktası belirtebilirsiniz:
...
Sunucu ayarlarını `sumocode.json` yapılandırma dosyanızda da yapılandırabilirsiniz:

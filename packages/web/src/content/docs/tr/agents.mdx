...
SumoCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

SumoCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `sumocode.json` yapılandırma dosyanızda yapılandırın:
...
sumocode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [SumoCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `sumocode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `sumocode models` komutunu çalıştırın.
:::

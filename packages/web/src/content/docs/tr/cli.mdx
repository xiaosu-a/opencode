...
sumocode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.
...
sumocode terminal kullanıcı arayüzünü başlatır.
...
sumocode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

sumocode için agent'ları yönetin.
...
opencode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `sumocode auth login` kullanabilirsiniz. Bu `~/.local/share/opencode/auth.json`'de saklanıyor.

```bash
sumocode auth login
```

sumocode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `sumocode serve` örneğine de ekleyebilirsiniz:
...
sumocode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
sumocode CLI aşağıdaki global bayrakları destekler.
...
sumocode ortam değişkenleri kullanılarak yapılandırılabilir.

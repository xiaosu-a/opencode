---
title: Eklentiler
description: SumoCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek SumoCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya SumoCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.sumocode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/sumocode/plugins/` - Genel eklentiler
...
1. Global config (`~/.config/sumocode/sumocode.json`)
2. Project config (`sumocode.json`)
3. Global eklenti dizini (`~/.config/sumocode/plugins/`)
4. Proje eklenti dizini (`.sumocode/plugins/`)
...
SumoCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir SumoCode SDK istemcisi.
...
SumoCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
SumoCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
SumoCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca SumoCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, SumoCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra SumoCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://sumocode.ai/docs/sdk) bakın.

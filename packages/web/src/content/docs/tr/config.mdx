...
3. **Özel yapılandırma** (`SUMOCODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `sumocode.json`) - projeye özgü ayarlar
5. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`SUMOCODE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.opencode` ve `~/.config/sumocode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
`SUMOCODE_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`SUMOCODE_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
...
Yapılandırma dosyası [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json)'da tanımlanan bir şemaya sahiptir.

...
:::note
Bu araç yalnızca `SUMOCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `SUMOCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca opencode provider kullanılırken veya `SUMOCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

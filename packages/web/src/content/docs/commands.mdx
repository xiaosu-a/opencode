...
Create `.sumocode/commands/test.md`:
...
You can add custom commands through the SumoCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your SumoCode [config](/docs/config):
...
sumocode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).

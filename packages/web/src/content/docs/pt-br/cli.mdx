...
O opencode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `sumocode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.
...
```bash "opencode run"
sumocode run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `sumocode serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
sumocode run --attach http://localhost:4096 "Explique async/await em JavaScript"
```

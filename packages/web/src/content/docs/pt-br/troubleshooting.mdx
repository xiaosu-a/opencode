...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `sumocode --log-level DEBUG`.
...
- Apenas macOS: menu `SumoCode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
Se seu `sumocode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
3. Certifique-se de que você tem a versão mais recente com `sumocode upgrade`
...
Para descobrir quais modelos você tem acesso, execute `sumocode models`

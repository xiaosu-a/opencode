...
2. **Configuração global** (`~/.config/sumocode/sumocode.json`) - preferências do usuário
3. **Configuração personalizada** (`SUMOCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`sumocode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.opencode`** - agentes, comandos, plugins
6. **Configuração inline** (`SUMOCODE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.opencode` e `~/.config/sumocode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/sumocode/sumocode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/sumocode/tui.json`.
...
Adicione `sumocode.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `SUMOCODE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `SUMOCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).

A configuração do TUI usa [**`sumocode.ai/tui.json`**](https://sumocode.ai/tui.json).
...
Use `SUMOCODE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `sumocode.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/sumocode/agents/` ou `.sumocode/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/sumocode/commands/` ou `.sumocode/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.sumocode/plugins/` ou `~/.config/sumocode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.

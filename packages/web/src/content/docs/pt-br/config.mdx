...
Você pode configurar as configurações do servidor para os comandos `sumocode serve` e `sumocode web` através da opção `server`.
...
Essa configuração se aplica a todas as interfaces: TUI, CLI (`sumocode run`), aplicativo desktop e GitHub Action.

...
sumocode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
SumoCode Zen é uma lista de modelos fornecidos pela equipe do opencode que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o SumoCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `SumoCode Zen` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
SumoCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do opencode que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `SumoCode Go` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `sumocode.json`:
...
sumocode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
...
arquivo `sumocode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `sumocode.json`:
...
SumoCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do opencode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>SumoCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **SumoCode Zen**.
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no sumocode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `sumocode.json` no diretório do seu projeto:
...
1. **Verifique a configuração de autenticação**: Execute `sumocode auth list` para ver se as credenciais

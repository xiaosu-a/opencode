---
title: Plugins
description: Write your own plugins to extend SumoCode.
---

Plugins allow you to extend SumoCode by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify SumoCode's default behavior.
...
- `.sumocode/plugins/` - Project-level plugins
- `~/.config/sumocode/plugins/` - Global plugins
...
3. Global plugin directory (`~/.config/sumocode/plugins/`)
4. Project plugin directory (`.sumocode/plugins/`)
...
SumoCode runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the SumoCode desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://sumocode.ai/docs/sdk) for details.

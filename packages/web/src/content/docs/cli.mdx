...
SumoCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `sumocode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.
...
You can also attach to a running `sumocode serve` instance to avoid MCP server cold boot times on every run:
...
```bash
sumocode session delete <sessionID>
```
...
This starts an HTTP server and opens a web browser to access SumoCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
```bash
sumocode plugin <module>
```

Or use the alias.

```bash
sumocode plug <module>
```

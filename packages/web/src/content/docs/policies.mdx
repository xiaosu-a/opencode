---
title: Policies
description: Control which configured resources SumoCode may use.
---

Policies control whether SumoCode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `sumocode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether SumoCode may use a resource such as an LLM provider.
...
SumoCode currently supports one policy action:

...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `sumocode --log-level DEBUG`.

---

## Przechowywanie danych

sumocode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
sumocode Desktop uruchamia w tle lokalny serwer opencode (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `SumoCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/sumocode/sumocode.jsonc` (lub `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/sumocode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
sumocode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
sumocode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `sumocode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
sumocode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:
...
3. Upewnij się, że masz najnowszą wersję z `sumocode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `sumocode models`
...
sumocode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.

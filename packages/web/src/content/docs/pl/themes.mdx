...
sumocode ma kilka wbudowanych motywów.
...
- Chcą, aby SumoCode pasował do wyglądu ich terminala
...
sumocode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/sumocode/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.sumocode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.sumocode/themes/*.json`

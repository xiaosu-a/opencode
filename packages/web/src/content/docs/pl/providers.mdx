...
sumocode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.
...
SumoCode Zen to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od SumoCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `SumoCode Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
SumoCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `SumoCode Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `sumocode.json`:
...
sumocode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
...
`sumocode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
SumoCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>SumoCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **SumoCode Zen**.
...
4. Utwórz lub zaktualizuj plik `sumocode.json` w katalogu projektu:
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `sumocode auth list`, aby sprawdzić, czy poświadczenia

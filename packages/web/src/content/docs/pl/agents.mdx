...
W SumoCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
SumoCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
SumoCode ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

SumoCode ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej SumoCode, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `sumocode.json`:
...
- Globalnie: `~/.config/sumocode/agents/`
- W projekcie: `.sumocode/agents/`

```markdown title="~/.config/sumocode/agents/review.md"
---
...
Jeśli nie zostanie skonfigurowana, SumoCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji SumoCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji SumoCode w `provider/model-id`. Na przykład, [SumoCode Zen](/docs/zen), użyj `sumocode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
```json title="sumocode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="sumocode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Uruchom `sumocode models`, aby zobaczyć dostępne modele.
:::
...
```markdown title="~/.config/sumocode/agents/docs-writer.md"
---
...
```markdown title="~/.config/sumocode/agents/security-auditor.md"
---

---
title: Konfiguracja
description: Ustawienia SumoCode w pliku JSON.
---

Możesz dostosować SumoCode za pomocą pliku konfiguracyjnego JSON.

---

## Format

SumoCode obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
2. **Konfiguracja globalna** (`~/.config/sumocode/sumocode.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `SUMOCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`sumocode.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.opencode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `SUMOCODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.opencode` i `~/.config/sumocode` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację SumoCode w `~/.config/sumocode/sumocode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/sumocode/tui.json`.
...
Dodaj `sumocode.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy SumoCode się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `SUMOCODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `SUMOCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`sumocode.ai/config.json`**](https://sumocode.ai/config.json).
...
Użyj `SUMOCODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `sumocode.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer SumoCode.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie SumoCode wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/sumocode/agents/` lub `.sumocode/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, SumoCode powróci do `"build"` z ostrzeżeniem.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/sumocode/commands/` lub `.sumocode/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
SumoCode może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy SumoCode nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie SumoCode **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają SumoCode o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.sumocode/plugins/` lub `~/.config/sumocode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie SumoCode tylko do określonych dostawców.

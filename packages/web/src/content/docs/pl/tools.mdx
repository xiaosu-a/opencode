...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. SumoCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w SumoCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SUMOCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `SUMOCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy SumoCode lub gdy zmienna środowiskowa `SUMOCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu SumoCode:

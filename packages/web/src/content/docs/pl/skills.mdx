...
sumocode przeszukuje te lokalizacje:
...
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.sumocode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/sumocode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.sumocode/skills/git-release/SKILL.md` w ten sposób:
...
sumocode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `sumocode.json`:
...
**Dla agentów wbudowanych** (w `sumocode.json`):

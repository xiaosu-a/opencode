---
title: Providers
description: Using any LLM provider in SumoCode.
---

import config from "../../../config.mjs"
export const console = config.console

SumoCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your SumoCode config.
...
You can customize the providers through the `provider` section in your SumoCode
config.
...
SumoCode Zen is a list of models provided by the SumoCode team that have been
tested and verified to work well with SumoCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with SumoCode Zen.
:::

1. Run the `/connect` command in the TUI, select `SumoCode Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in SumoCode and is completely optional to use.

---

## SumoCode Go

SumoCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the SumoCode team that have been
tested and verified to work well with SumoCode.

1. Run the `/connect` command in the TUI, select `SumoCode Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with SumoCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `sumocode.json`:
...
SumoCode. Anthropic explicitly prohibits this.

Previous versions of SumoCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in SumoCode with zero setup:
...
```json title="sumocode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
SumoCode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; SumoCode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize SumoCode. Sign in and approve.

   :::note
   SumoCode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, SumoCode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — SumoCode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in SumoCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

SumoCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
SumoCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock SumoCode
to only use your own GitLab-hosted instance, add the following to your
`sumocode.json` file. It is also recommended to disable session sharing.
...
Customize through `sumocode.json`:
...
When a `duo-workflow-*` model is selected, SumoCode will:
...
4. Route tool execution requests through SumoCode's permission-gated tool system
...
To use GMI Cloud with SumoCode:
...
To use Google Vertex AI with SumoCode:
...
```jsonc title="~/.config/sumocode/sumocode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each SumoCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each SumoCode conversation listed as a separate session.
...
```json title="sumocode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="sumocode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
:::tip
Ollama can automatically configure itself for SumoCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="sumocode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with SumoCode:
...
3. Copy the API key for use in SumoCode.
...
6. **Important**: Before using cloud models in SumoCode, you must pull the model information locally:
...
SumoCode Zen is a list of tested and verified models provided by the SumoCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>SumoCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **SumoCode Zen**.

   ```txt
   /connect
   ```

3. Enter your SumoCode API key.
...
5. Complete sign-in in the browser that opens. SumoCode captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
The model catalog is provided automatically. A minimal `sumocode.json` is all that's needed:
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. SumoCode opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
SumoCode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when SumoCode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running SumoCode — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. SumoCode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. SumoCode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in sumocode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `sumocode.json` file in your project directory:

   ```json title="sumocode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="sumocode.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow SumoCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `sumocode auth list` to see if the credentials

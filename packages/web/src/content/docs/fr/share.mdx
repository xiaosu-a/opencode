---
title: Partager
description: Partagez vos conversations SumoCode.
---

La fonctionnalité de partage de SumoCode vous permet de créer des liens publics vers vos conversations SumoCode, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, SumoCode :
...
SumoCode prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, SumoCode utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
...
Pour appliquer cela au sein de votre équipe pour un projet donné, ajoutez-le au `sumocode.json` dans votre projet et enregistrez-vous dans Git.

...
Pour déboguer les problèmes avec SumoCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `sumocode --log-level DEBUG`.

---

## Stockage

sumocode stocke les données de session et autres données d'application sur le disque à l'emplacement :
...
SumoCode Desktop exécute un serveur SumoCode local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `SumoCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/sumocode/sumocode.jsonc` (ou `~/.config/sumocode/sumocode.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/sumocode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\sumocode.jsonc`
...
SumoCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que SumoCode puisse le reconstruire.

1. Quittez complètement SumoCode Desktop.
...
3. Redémarrez le bureau SumoCode.

---

### Résoudre les problèmes de connexion au serveur

SumoCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `sumocode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, SumoCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de SumoCode.

---

### Les notifications ne s'affichent pas

SumoCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour SumoCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau SumoCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application SumoCode Desktop) :
...
Si vous rencontrez des problèmes avec SumoCode :
...
3. Assurez-vous d'avoir la dernière version avec `sumocode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `sumocode models`
...
sumocode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.

---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en SumoCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SumoCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration SumoCode.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre SumoCode
configuration.
...
SumoCode Zen est une liste de modèles fournis par l'équipe SumoCode qui ont été
testé et vérifié pour fonctionner correctement avec SumoCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par SumoCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez opencode et dirigez-vous vers [sumocode.ai/auth](https://sumocode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans SumoCode et son utilisation est totalement facultative.

---

## SumoCode Go

SumoCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe SumoCode qui ont été
testé et vérifié pour fonctionner correctement avec SumoCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `SumoCode Go` et rendez-vous sur [sumocode.ai/auth](https://sumocode.ai/zen).
...
Pour utiliser Amazon Bedrock avec SumoCode :
...
Pour une configuration spécifique au projet ou persistante, utilisez `sumocode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans SumoCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
SumoCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller SumoCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `sumocode.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `sumocode.json` :
...
Pour utiliser Google Vertex AI avec SumoCode :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation SumoCode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation SumoCode répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour SumoCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec SumoCode :
...
3. Copiez la clé API à utiliser dans SumoCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans SumoCode, vous devez extraire les informations du modèle localement :
...
SumoCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe SumoCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>SumoCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **SumoCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé SumoCode API.
...
4. Créez ou mettez à jour votre fichier `sumocode.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à SumoCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.

...
Avec SumoCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, SumoCode utilise notre propre thème `opencode`.
...
SumoCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que SumoCode corresponde à l'apparence de leur terminal
...
SumoCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/sumocode/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.sumocode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.sumocode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/sumocode/themes
vim ~/.config/sumocode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .sumocode/themes
vim .sumocode/themes/my-theme.json
```
...
{
  "$schema": "https://sumocode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

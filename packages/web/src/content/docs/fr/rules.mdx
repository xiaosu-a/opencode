...
Vous pouvez également avoir des règles globales dans un fichier `~/.config/sumocode/AGENTS.md`. Ceci est appliqué à toutes les sessions opencode.
...
Pour les utilisateurs migrant depuis Claude Code, SumoCode prend en charge les conventions de fichiers de Claude Code comme solutions de secours :

- **Règles du projet** : `CLAUDE.md` dans le répertoire de votre projet (utilisé si aucun `AGENTS.md` n'existe)
- **Règles globales** : `~/.claude/CLAUDE.md` (utilisé si aucun `~/.config/sumocode/AGENTS.md` n'existe)
...
2. **Fichier global** à `~/.config/sumocode/AGENTS.md`
3. **Fichier Claude Code** à `~/.claude/CLAUDE.md` (sauf si désactivé)

Le premier fichier correspondant gagne dans chaque catégorie. Par exemple, si vous possédez à la fois `AGENTS.md` et `CLAUDE.md`, seul `AGENTS.md` est utilisé. De même, `~/.config/sumocode/AGENTS.md` a priorité sur `~/.claude/CLAUDE.md`.

---

## Instructions personnalisées

Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `sumocode.json` ou dans le `~/.config/sumocode/sumocode.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de sumocode.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `sumocode.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `sumocode.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::

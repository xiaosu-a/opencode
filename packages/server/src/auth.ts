...
export class Config extends Context.Service<Config, Info>()("@sumocode/ServerAuthConfig") {
...
export function header(credentials?: Credentials) {
  ...
  return `Basic ${Buffer.from(`${credentials?.username ?? process.env.SUMOCODE_SERVER_USERNAME ?? "opencode"}:${password}`).toString("base64")}`
}

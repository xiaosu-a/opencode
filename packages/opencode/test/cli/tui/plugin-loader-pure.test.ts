...
`,
  ...
  const pure = process.env.SUMOCODE_PURE
  const meta = process.env.SUMOCODE_PLUGIN_META_FILE
  process.env.SUMOCODE_PURE = "1"
  process.env.SUMOCODE_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.SUMOCODE_PURE
    } else {
      process.env.SUMOCODE_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.SUMOCODE_PLUGIN_META_FILE
    } else {
      process.env.SUMOCODE_PLUGIN_META_FILE = meta
    }
  }
})

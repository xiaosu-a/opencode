...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "sumocode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "sumocode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sumocode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sumocode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["sumocode.json", "sumocode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "sumocode.json"),
        path.join(project, "sumocode.jsonc"),
      ])
    })
  })

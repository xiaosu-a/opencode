...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`SumoCode is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export SUMOCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SUMOCODE_CLIENT=desktop",
    `export SUMOCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export SUMOCODE_SERVER_PASSWORD=${shellEscape(password)}`,

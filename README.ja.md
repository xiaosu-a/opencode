...
SumoCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/opencode/releases) から直接ダウンロードするか、[sumocode.ai/download](https://sumocode.ai/download) を利用してください。
...
1. `$SUMOCODE_INSTALL_DIR` - カスタムのインストールディレクトリ
...
[agents](https://sumocode.ai/docs/agents) の詳細はこちら。

### ドキュメント

SumoCode の設定については [**ドキュメント**](https://sumocode.ai/docs) を参照してください。
...
SumoCode に関連するプロジェクトで、名前に "opencode"（例: "opencode-dashboard" や "opencode-mobile"）を含める場合は、そのプロジェクトが SumoCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。
